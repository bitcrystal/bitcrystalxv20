...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BTCRY")
    updateDisplayUnit();
}
...
{
    QString ret = QString("BitCrystalX:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BTCRY unit input in lnReqAmount, we generate the URI with BTCRY as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BTCRY, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
